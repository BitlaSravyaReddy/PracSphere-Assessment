...
export default function ProfilePage() {
  ...
  // Validate username parameter and handle URL actions
  useEffect(() => {
    if (status === "authenticated" && session?.user) {
      ...
      if (urlUsername !== expectedSlug) {
        router.replace(`/profile/${expectedSlug}`);
        return;
      }

      // Handle URL search params for actions
      const action = searchParams.get('action');
      const field = searchParams.get('field');

      if (action === 'editing' && field === 'name') {
        setEditMode(true);
  ...
  }, [status, session, params.username, searchParams, router]);
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Clear URL params
        const username = getUserSlug(profileForm.name, session?.user?.email);
                ...
                {!editMode && (
                  <button
                    onClick={() => {
                      ...
                      router.push(`/profile/${username}?action=editing&field=name`);
                    }}
                    style={{
                      padding: '0.5rem 1rem',
